...
This is a Kubernetes operator for the Inference Gateway project, built using Go 1.26.2+ and the Operator SDK. The operator manages Gateway, A2A (Agent-to-Agent), and MCP (Model Context Protocol) custom resources in Kubernetes clusters.
...
- `task cluster:create` - Create local k3d cluster with cert-manager and Envoy Gateway (Gateway API)
...
## Naming convention: project Gateway vs upstream Gateway API

This project's CRD `Gateway` (`core.inference-gateway.com/v1alpha1`) shares its short name with the Kubernetes Gateway API's `Gateway` (`gateway.networking.k8s.io/v1`). In Go code:

- Import the upstream API as `gwapiv1 "sigs.k8s.io/gateway-api/apis/v1"`.
- Reserve the local variable name `gateway` for the project CR (`*corev1alpha1.Gateway`). For upstream objects use `gw` or descriptive names (`upstreamGateway`, `httpRoute`).
- In YAML, distinguish with the fully-qualified plural: `gateway.core.inference-gateway.com` vs `gateway.gateway.networking.k8s.io`.

The operator-managed `Gateway` CR has a `spec.routing` field that drives reconciliation of upstream `Gateway` and `HTTPRoute` resources (Gateway API). The legacy `spec.ingress` field has been removed; `task cluster:create` now installs Envoy Gateway and the Gateway API CRDs in place of nginx-ingress.
